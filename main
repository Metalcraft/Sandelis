        ...
        "stages": [
            {
                "name": e.pavadinimas,
                "total": e.is_viso,
                "collected": e.surinkta_sk,
                "delivered": e.perduota_sk,
            }
            for e in arch
        ]
    }
...
def get_detales(uzs_id: str, db: Session = Depends(get_db)):
    items = db.query(Detale).filter(Detale.uzsakymo_id == uzs_id).order_by(
        Detale.storis, Detale.pavadinimas
    ).all()
...
def add_detale(data: dict, db: Session = Depends(get_db)):
    ...
    d = Detale(
        det_id=det_id,
        uzsakymo_id=data["uzsakymoId"],
        pavadinimas=data.get("pavadinimas", "Detale"),
        storis=storis,
        plotas=plotas,
        kiekis=kiekis,
        svoris=svoris,
        konturas=data.get("konturas", ""),
    )
    db.add(d)
    db.commit()
...
def update_detale(det_id: str, data: dict, db: Session = Depends(get_db)):
    ...
    if "storis" in data:
        d.storis = float(data["storis"])
    if "kiekis" in data:
        d.kiekis = int(data["kiekis"])
    if "svoris" in data:
...
def naudoti(stk_id: str, data: dict, db: Session = Depends(get_db)):
    ...
    hist = SandelioIstorijia(
        ...
        matmenys=s.matmenys,
        kiekis=qty,
        svoris_vnt=s.svoris_vnt,
        svoris_is_viso=round(qty * s.svoris_vnt, 2),
        kaina_kg=s.kaina_kg,
        verte=round((qty * s.svoris_vnt / 1000) * s.kaina_kg, 2),
        pastabos=data.get("pastabos", ""),
    )
    db.add(hist)
    db.commit()
...
def get_istorija(db: Session = Depends(get_db)):
    ...
    return {
        "history": [
            {
                "data": h.data.strftime("%Y-%m-%d %H:%M"),
                "veiksmas": h.veiksmas,
                "storis": h.storis,
                "matmenys": h.matmenys,
                "kiekis": h.kiekis,
                "svorisVnt": h.svoris_vnt,
                "svorisIsViso": h.svoris_is_viso,
                "kainaKg": h.kaina_kg,
                "verte": h.verte,
            }
            for h in items
...
def ataskaita(nuo: str, iki: str, db: Session = Depends(get_db)):
    ...
    return {
        "lakstai": {
            "gauta": lk_gauta,
            "surinkta": lk_surinkta,
            "perduota": lk_perduota,
        },
        "dxf": {
            "sk": len(uzs),
            "svoris": round(sum(u.bendras_svoris for u in uzs), 3),
        },
        "sandelis": {
...
async def siusti_email(db: Session = Depends(get_db)):
    ...
    def rows(lst, color):
        if not lst:
            return "<tr><td colspan='2' style='color:#aaa'>Tuscia</td></tr>"
        return "".join(
            f"<tr><td style='padding:3px 8px;border-bottom:1px solid #eee'>{l.kodas}</td>"
            f"<td style='padding:3px 8px;color:{color}'>{l.etapas or ''}</td></tr>"
            for l in lst
        )
...
def _uzs(u):
    return {
        "id": u.uzs_id,
        "klientas": u.klientas,
        "aprasymas": u.aprasymas or "",
        "pastabos": u.pastabos or "",
        "statusas": u.statusas,
        "bendraSvoris": u.bendras_svoris,
        "detaliuSk": u.detaliu_sk,
...
def _stk(s):
    return {
        ...
        "storis": s.storis,
        "matmenys": s.matmenys,
        "svorisVnt": s.svoris_vnt,
        "gautaVnt": s.gauta_vnt,
        "sunaudotaVnt": s.sunaudota_vnt,
        "likoVnt": s.liko_vnt,
        "likoKg": s.liko_kg,
        "likoT": s.liko_t,
        "kainaKg": s.kaina_kg,
