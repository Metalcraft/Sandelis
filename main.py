...
def _det(d):
    kkg = getattr(d, "kaina_kg", None) or 1.55
...
@app.post("/api/lazeris/preke")
def add_lazeris_preke(data: dict, db: Session = Depends(get_db)):
    import uuid
    preke_id = "LAZ-" + str(uuid.uuid4())[:8].upper()
    p = LazerisPreke(preke_id=preke_id, tipas=data.get("tipas","galvute"),
                     dydis=data.get("dydis",""), pavadinimas=data.get("pavadinimas",""),
                     kiekis=int(data.get("kiekis",0)), min_kiekis=int(data.get("minKiekis",2)))
    db.add(p); db.commit()
    return {"success": True, "id": preke_id}

@app.put("/api/lazeris/preke/{preke_id}")
def update_lazeris_preke(preke_id: str, data: dict, db: Session = Depends(get_db)):
    p = db.query(LazerisPreke).filter(LazerisPreke.preke_id == preke_id).first()
    if not p: raise HTTPException(404)
    if "kiekis" in data: p.kiekis = int(data["kiekis"])
    if "minKiekis" in data: p.min_kiekis = int(data["minKiekis"])
    if "pavadinimas" in data: p.pavadinimas = data["pavadinimas"]
    db.commit()
    return {"success": True}
...
def paimti_lazeris(preke_id: str, data: dict, db: Session = Depends(get_db)):
    ...
    if not p: raise HTTPException(404)
    kiekis = int(data.get("kiekis", 1))
    p.kiekis = max(0, p.kiekis - kiekis)
    hist = LazerisIstorijia(preke_id=preke_id, veiksmas="paimta", kiekis=kiekis, likutis=p.kiekis)
    db.add(hist); db.commit()
    return {"success": True, "kiekis": p.kiekis, "mazasLikutis": p.kiekis <= p.min_kiekis}

@app.post("/api/lazeris/preke/{preke_id}/prideti")
def prideti_lazeris(preke_id: str, data: dict, db: Session = Depends(get_db)):
    p = db.query(LazerisPreke).filter(LazerisPreke.preke_id == preke_id).first()
    if not p: raise HTTPException(404)
    kiekis = int(data.get("kiekis", 1))
    p.kiekis += kiekis
    hist = LazerisIstorijia(preke_id=preke_id, veiksmas="prideta", kiekis=kiekis, likutis=p.kiekis)
    db.add(hist); db.commit()
    return {"success": True, "kiekis": p.kiekis}

@app.delete("/api/lazeris/preke/{preke_id}")
def delete_lazeris_preke(preke_id: str, db: Session = Depends(get_db)):
    p = db.query(LazerisPreke).filter(LazerisPreke.preke_id == preke_id).first()
    if not p: raise HTTPException(404)
    db.delete(p); db.commit()
    return {"success": True}

@app.get("/api/lazeris/istorija")
def get_lazeris_istorija(db: Session = Depends(get_db)):
    items = db.query(LazerisIstorijia).order_by(LazerisIstorijia.data.desc()).limit(100).all()
    return {"istorija": [{"id": h.id, "prekeId": h.preke_id, "veiksmas": h.veiksmas,
                          "kiekis": h.kiekis, "likutis": h.likutis,
                          "data": h.data.strftime("%Y-%m-%d %H:%M")} for h in items]}
...
def auth_login(data: dict):
    ...
    if slaptazodis == ADMIN_PASS:
        ...
        TOKENS[token] = {"role": "admin", "vardas": "Admin"}
        return {"token": token, "role": "admin", "vardas": "Admin"}
    if slaptazodis == WORKER_PIN:
        token = _secrets.token_hex(16)
        TOKENS[token] = {"role": "darbuotojas", "vardas": "Darbuotojas"}
        return {"token": token, "role": "darbuotojas", "vardas": "Darbuotojas"}
    return {"klaida": "Neteisingas slaptažodis ar PIN"}

@app.get("/api/auth/me")
def auth_me(token: str = ""):
    if token in TOKENS:
        t = TOKENS[token]
        return {"ok": True, "role": t["role"], "vardas": t["vardas"]}
...
def change_pass(data: dict):
    global ADMIN_PASS
    naujas = data.get("naujas", "").strip()
    if not naujas or len(naujas) < 4:
        return {"klaida": "Per trumpas slaptažodis (min. 4 simboliai)"}
    ADMIN_PASS = naujas
    return {"ok": True}
...
def change_pin(data: dict):
    ...
    naujas = data.get("naujas", "").strip()
    if not naujas or len(naujas) < 4:
        return {"klaida": "Per trumpas PIN (min. 4 simboliai)"}
    WORKER_PIN = naujas
    return {"ok": True}from fastapi import FastAPI, Depends, HTTPException
from fastapi.responses import HTMLResponse, JSONResponse
from fastapi.staticfiles import StaticFiles
from fastapi.templating import Jinja2Templates
from fastapi import Request
